# Phase 23: Independent Thread Scheduling Compute Graph IR — Design Document / 设计文档

> **对应 GPGPU-Sim**: CUDA Graphs API (cudaGraph)
> **参考**: NVIDIA CUDA Graphs (docs.nvidia.com/cuda/cuda-c-programming-guide),
> **Phase 23 新增**: Per-thread PC model with independent thread scheduling and hardware-managed reconvergence. Each thread has its own program counter (Volta+ style). Scheduler tracks per-thread state instead of per-warp. Inspired by NVIDIA Volta Independent Thread Scheduling.
...
| `learning_console.py` | Interactive console — new `graph` command (Phase 23) | ADDED: `graph` command + `print_graph_info()` |
