...
- **GraphExecutor**: topological-order execution, critical path analysis, concurrent group detection
- **Kernel Fusion** (): merge adjacent element-wise kernels to reduce launch overhead
- **Memory Planning** (): first-fit buffer allocation with reuse
- **Concurrent Execution**: level-based grouping for parallel kernel launch
- 7 test cases, all pass | No new opcodes
