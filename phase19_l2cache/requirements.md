# Phase 19: 学习控制台需求 + Warp 通信 + 分块策略 + CuTile 编程模型 + Compute Graph IR

## New Features (2026-05-16) — Phase 15

The following Compute Graph IR features were added in Phase 19:
...
- **Compute Graph IR** (`src/graph_ir.py`, ~213 lines): A DAG-based intermediate representation for computation graphs, inspired by NVIDIA CUDA Graphs, XLA HLO, and MLIR Graph Dialect.
  ...
  - **ComputeGraph**: DAG manager with node addition, dependency management, validation, topological sort, and serialization.
  - **Five node types**: `kernel` (GPU compute), `memcpy` (device data transfer), `barrier` (synchronization point), `input` (graph entry data), `output` (graph exit data).

- **DAG Validation** — `ComputeGraph.validate()`:
  - Three-color DFS (White-Gray-Black) cycle detection
  - Missing dependency node checking
  - Empty graph detection
  - Returns `(bool, message)` tuple: `(True, "Valid DAG")` or `(False, "Cycle detected at node X")`

- **Topological Sort** — `ComputeGraph.topological_order()`:
  - Kahn's algorithm (in-degree based BFS)
  - Returns list of node IDs in valid execution order
  - If graph has cycles, returns partial order (missing nodes indicate cycle)

- **JSON Serialization** — `ComputeGraph.to_json()` / `ComputeGraph.from_json()`:
  - Structured JSON format with `name` and `nodes` fields
  - Each node contains `id`, `type`, `name`, `params`, `deps`
  - Human-readable, cross-language compatible
  - Round-trip: to_json() → from_json() preserves graph structure

- **DOT Visualization** — `ComputeGraph.to_dot()`:
  - Graphviz DOT format output
  - Node shapes: kernel=box, memcpy=ellipse, barrier=diamond, input=invhouse, output=house
  - Dependency edges: `n_dep -> n_node`
  - Compatible with any Graphviz viewer (graphviz.org)

- **Example graph builder** — `build_example_graph()`:
  - Creates a simple pipeline: kernel_A → kernel_B → barrier → kernel_C
  - Validates the DAG and returns the graph
  - Used for demonstration and testing

- **Zero ISA changes**: Phase 15 is a pure software IR layer. No new opcodes, no new assembly mnemonics, no pipeline modifications.

- **Zero module dependencies**: `graph_ir.py` only uses Python standard library (`dataclasses`, `typing`, `json`). No dependency on other toygpgpu modules.

- **Full backward compatibility**: All existing Phase 0-14 programs and tests run unchanged.

- **New console command**: `graph` command in `learning_console.py` prints graph IR information including example graph construction, validation, topological order, and DOT output.

- **CUDA Graphs correspondence**:
  - `ComputeGraph(name)` = `cudaGraphCreate`
  - `add_kernel()` = `cudaGraphAddKernelNode`
  - `add_memcpy()` = `cudaGraphAddMemcpyNode`
  - `add_barrier()` = `cudaGraphAddEmptyNode`
  - `validate()` = `cudaGraphInstantiate` (validation phase)
  - `to_json()` = `cudaGraphExportToJSON` (conceptual)

- **XLA/MLIR correspondence**:
  - `ComputeGraph` = XLA HLO Module / MLIR Graph Region
  - `GraphNode` = XLA HLO Instruction / MLIR Operation
  - `to_dot()` = MLIR Graphviz output (conceptual)
