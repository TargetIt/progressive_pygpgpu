# Phase 20: Tensor Memory Accelerator (TMA) Compute Graph IR — Design Document / 设计文档

> **对应 GPGPU-Sim**: CUDA Graphs API (cudaGraph)
> **参考**: NVIDIA CUDA Graphs (docs.nvidia.com/cuda/cuda-c-programming-guide),
> **Phase 20 新增**: TMA: hardware unit for async tensor address computation and data movement. Offloads address generation from threads. Inspired by NVIDIA Hopper TMA. Supports multi-dimensional tensor descriptors and background DMA transfers.
...
| `learning_console.py` | Interactive console — new `graph` command (Phase 20) | ADDED: `graph` command + `print_graph_info()` |
