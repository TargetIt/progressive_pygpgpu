...
class SIMTCore:
    ...
    def __init__(self, warp_size: int = DEFAULT_WARP_SIZE,
        ...
        self.l2_cache = L2Cache()               # L2 shared cache (Phase 19)
        self.bw_model = BandwidthModel()
        self.tma = TMAEngine()       # bandwidth model (Phase 19)
    ...
    def _mem_load(self, warp, instr):
        ...
        if self._is_contiguous(addrs):
            ...
            for i in range(start, start + 4):
                ...
                if val is None:
                    val, lat = self.l2_cache.read(i)
                    if val is None:
                        val = self.memory.read_word(i)
                        self.l2_cache.fill_line(i & ~3, [self.memory.read_word(j) for j in range(i & ~3, (i & ~3) + 4)])
            ...
            for t in tlist:
                ...
                if val is None:
                    val, lat = self.l2_cache.read(addr)
                    if val is None:
    ...
    def dump_state(self) -> str:
        ...
        lines.append(self.l2_cache.stats())
        lines.append(self.bw_model.report(self.l2_cache))
