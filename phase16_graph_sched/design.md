# Phase 16: Graph Scheduler & Executor Compute Graph IR — Design Document / 设计文档

> **对应 GPGPU-Sim**: CUDA Graphs API (cudaGraph)
> **参考**: NVIDIA CUDA Graphs (docs.nvidia.com/cuda/cuda-c-programming-guide),
> **Phase 16 新增**: GraphExecutor: topological-order execution with kernel fusion (element-wise), critical path analysis, concurrent group detection, memory lifecycle planning, and graph replay. Builds on Phase 15's Compute Graph IR to make graphs executable.
...
| `learning_console.py` | Interactive console — new `graph` command (Phase 16) | ADDED: `graph` command + `print_graph_info()` |
