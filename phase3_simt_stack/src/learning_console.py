#!/usr/bin/env python3
"""
Learning Console - Interactive GPU Pipeline Debugger (Phase 3: SIMT Stack)
===========================================================================
Step-by-step inspection of the SIMT core with branch divergence tracking.
...
def print_state(simt):
    ...
    for w in simt.warps:
        ...
        print(f"  Warp {w.warp_id}: PC={w.pc}, mask=0b{mask_str}, {status}")
...
def print_changes(simt, cycle, instr_info, warp_info,
    ...
    if old_stack_entries != new_stack_entries:
        if len(new_stack_entries) > len(old_stack_entries):
            # Push happened: entry = (warp_id, reconv_pc, orig_mask, taken_mask, fallthrough_pc)
...
def run_console(simt, program_text, args):
    ...
    # Header
    print("==============================================")
    print("  toygpgpu Learning Console (Phase 3)")
    print("  SIMT Stack / Branch Divergence Debugger")
    print("==============================================")
    ...
    while cycle < max_cycles:
        ...
        for w in simt.warps:
            ...
            if w.pc != pre_pc.get(wid, -1) or w.active_mask != pre_mask.get(wid, 0):
                ...
                if pre_reconv.get(wid, False):
                    ...
                    # Normal execution: instruction at old PC was executed
                    old_pc = pre_pc[wid]
        ...
        if reconverged:
            print_changes(simt, cycle, None, exec_warp,
                          old_regs, new_regs, old_mem, new_mem,
                          old_stack_entries, new_stack_entries)
            ...
            print_changes(simt, cycle, exec_instr_info, exec_warp,
                          old_regs, new_regs, old_mem, new_mem,
                          old_stack_entries, new_stack_entries)
...
def main():
    ...
    if args['auto']:
        ...
        simt._t_stack_len = {w.warp_id: len(w.simt_stack.entries) for w in simt.warps}
        print(f"Auto-run with trace (max {cycle_limit} cycles)")
        print(f"Program: {len(prog)} instructions, "
              f"Config: {args['num_warps']} warp(s) x {args['warp_size']} threads/warp")
        print()
        while cycle < cycle_limit and simt.step():
