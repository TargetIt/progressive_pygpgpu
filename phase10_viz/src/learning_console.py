#!/usr/bin/env python3
"""
Learning Console -- Interactive GPU Pipeline Learning Console (Phase 10)
=========================================================================
Visualization support: trace events, timeline visualization,
stall analysis, memory heatmap.
...
Commands:
  ...
  trace       Print trace event buffer
  timeline    Print warp timeline visualization
  mma         Print MMA / tensor register details
  report      Print full execution report
  q           Quit
"""
        ...
        for t in w.active_threads():
            ...
            for ri in range(16):
                ...
                if val != 0:
                    ...
                    if lo != 0 or hi != 0:
                        if not printed:
                            print("  Warp %d Thread %d packed registers:" % (
                                w.warp_id, t.thread_id))
                            printed = True
                        print("    r%2d = 0x%08X  [lo=%d (0x%04X), hi=%d (0x%04X)]" % (
                            ri, val, lo, lo & 0xFFFF, hi, hi & 0xFFFF))


def print_trace_buffer(collector):
    """Print the trace event buffer"""
    from visualizer import TraceEvent
    if not collector or not collector.events:
        print("  (trace buffer empty - run program first)")
        return
    print(c("--- Trace Events ---", 'bold'))
    print("  Total cycles: %d, Events: %d" % (
        collector.total_cycles, len(collector.events)))
    print("  Stalls: %d (%.1f%%)" % (
        collector.stall_cycles,
        collector.stall_cycles / max(1, collector.total_cycles) * 100))
    print()
    # Print last 20 events (most recent at bottom)
    events = collector.events
    start = max(0, len(events) - 20)
    print("  %4s | %4s | %4s | %-8s | %s" % (
        'Cyc', 'Warp', 'PC', 'Opcode', 'Details'))
    print("  " + "-" * 50)
    for e in events[start:]:
        stall = ""
        if e.stall_reason:
            stall = " [stall: %s]" % e.stall_reason
        elif e.mem_addr >= 0:
            stall = " [addr=%d]" % e.mem_addr
        print("  %4d | %4d | %4d | %-8s | %d threads%s" % (
            e.cycle, e.warp_id, e.pc, e.opcode_name,
            e.active_threads, stall))


def print_timeline(collector, num_warps):
    """Print warp timeline from trace collector"""
    if not collector or not collector.events:
        print("  (no trace data - run program first)")
        return
    print(c("--- Warp Timeline ---", 'bold'))
    from visualizer import warp_timeline
    timeline = warp_timeline(collector.events, num_warps)
    print(timeline)


def print_stall_analysis(collector):
    """Print stall analysis from trace collector"""
    if not collector or not collector.events:
        print("  (no trace data - run program first)")
        return
    from visualizer import stall_analysis
    print(stall_analysis(collector.events))


def print_full_report(collector, num_warps, mem_size):
    """Print complete execution report"""
    if not collector or not collector.events:
        print("  (no trace data - run program first)")
        return
    from visualizer import full_report
    print(full_report(collector, num_warps, mem_size))
...
def run_console(simt, program_text, args):
    ...
    breakpoints = set()
    trace_collector = None  # Will hold TraceCollector after running via trace_runner

    prog = assemble(program_text)
    simt.load_program(prog)

    print(c("+-" + "=" * 50 + "-+", 'cyan'))
    print(c("|  toygpgpu Learning Console -- Interactive Debugger (Phase 10) |", 'cyan'))
    print(c("|  Visualization: trace events, timeline, stall analysis        |", 'cyan'))
    print(c("+-" + "=" * 50 + "-+", 'cyan'))
    ...
    print("|  trace/timeline/report = visualization commands")
    print(c("+-" + "=" * 50 + "-+", 'cyan'))
    ...
    while cycle < max_cycles:
        if auto_step:
            ...
            if cmd == '' or cmd.lower() == 's':
                ...
                # Run with trace to collect data if not already done
                if trace_collector is None:
                    from trace_runner import run_with_trace
                    # Reload and run with tracing
                    simt.load_program(prog)
                    trace_collector = run_with_trace(simt, max_cycles)
                    simt.load_program(prog)  # reload for normal use
                print_trace_buffer(trace_collector)
                continue
                ...
                if trace_collector is None:
                    ...
                    trace_collector = run_with_trace(simt, max_cycles)
                    simt.load_program(prog)
                print_timeline(trace_collector, num_warps)
                continue
                ...
                if trace_collector is None:
                    ...
                    trace_collector = run_with_trace(simt, max_cycles)
                    simt.load_program(prog)
                print_full_report(trace_collector, num_warps, 256)
                continue
