#!/usr/bin/env python3
"""
Learning Console -- Interactive GPU Pipeline Learning Console (Phase 8)
=======================================================================
PTX frontend support: can load .ptx files in addition to .asm files.
Shows PTX source alongside internal ISA translation.

Usage:
  python3 learning_console.py <program.asm|program.ptx> [options]
...
Commands:
  ...
  ptx         Print PTX source and ISA translation
  q           Quit
"""
...
def print_ptx_info(ptx_text, ptx_asm_text):
    """Print PTX source with its ISA translation"""
    print(c("--- PTX Source ---", 'bold'))
    for line in ptx_text.strip().split('\n'):
        print("  %s" % line)
    if ptx_asm_text:
        print()
        print(c("--- Translated ISA ---", 'bold'))
        for line in ptx_asm_text.strip().split('\n'):
            print("  %s" % line)


# ---- Console runner ----

def run_console(simt, program_text, args, source_type='asm', ptx_source=None, ptx_asm_text=None):
    ...
    # Load program (already assembled for .asm; for .ptx we assemble via ptx_parser)
    from assembler import assemble as asm_assemble
    if source_type == 'ptx' or source_type == 'ptx_loaded':
        # Program text is already machine code
        if isinstance(program_text, list):
            prog = program_text
        else:
            prog = asm_assemble(program_text)
    else:
        prog = asm_assemble(program_text)
    ...
    print(c("+-" + "=" * 50 + "-+", 'cyan'))
    print(c("|  toygpgpu Learning Console -- Interactive Debugger (Phase 8)  |", 'cyan'))
    print(c("|  PTX Frontend: supports .asm and .ptx input files             |", 'cyan'))
    print(c("+-" + "=" * 50 + "-+", 'cyan'))
    print("|  Program: %d instructions" % len(prog))
    print("|  Config:  %d warp(s) x %d threads/warp" % (num_warps, warp_size))
    print("|  Commands: Enter=step, r=run, i=info, q=quit, ptx=show PTX")
    print(c("+-" + "=" * 50 + "-+", 'cyan'))
    ...
    # If PTX was loaded, show translation
    if ptx_source:
        print(c("--- PTX Translation Summary ---", 'bold'))
        print("  PTX: %d lines -> ISA: %d instructions" % (
            len(ptx_source.strip().split('\n')), len(prog)))
        print("  Type 'ptx' to see full PTX source and translation")
        print()
    ...
    while cycle < max_cycles:
        if auto_step:
            ...
            if cmd == '' or cmd.lower() == 's':
                ...
                if ptx_source:
                    print_ptx_info(ptx_source, ptx_asm_text)
                else:
                    print("  No PTX source loaded (this is a .asm file)")
                continue
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 learning_console.py <program.asm|program.ptx> [options]")
    ...
    is_ptx = asm_file.lower().endswith('.ptx')
    ...
    if is_ptx:
        ...
        # Translate PTX to internal ISA
        from ptx_parser import assemble_ptx
        program, asm_text = assemble_ptx(ptx_source)
        ...
        if args.get('auto'):
            simt.load_program(program)
            simt.run(trace=True)
            return

        run_console(simt, program, args,
                    source_type='ptx_loaded',
                    ptx_source=ptx_source,
                    ptx_asm_text=asm_text)
    else:
        # Load .asm file
        with open(asm_file, encoding='utf-8') as f:
            program_text = f.read()
